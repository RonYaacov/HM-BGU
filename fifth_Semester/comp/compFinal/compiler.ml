...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair(ScmSymbol("if"),ScmPair(test, ScmPair(then_do, ScmPair(else_do, ScmNil)))) ->
    ScmIf(tag_parse test, tag_parse then_do, tag_parse else_do)
    | ScmPair(ScmSymbol("if"),ScmPair(test, ScmPair(then_do, ScmNil))) ->
    ScmIf(tag_parse test, tag_parse then_do, ScmConst(ScmVoid))
    ...
    | ScmPair(ScmSymbol "define", 
              ScmPair(ScmPair(ScmSymbol var, params), expr)) ->
    ...
    |ScmPair (ScmSymbol "let*", ScmPair (ribs, exprs)) ->
      ...
      |ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
        let ribs = (match(scheme_list_to_ocaml ribs) with
          | ribs, ScmNil -> 
            List.map (function
            | ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)) -> (var, expr)
            | _ -> raise (X_syntax "Malformed binding in letrec expression"))
            ribs
          | _ -> raise (X_syntax "malformed letrec-ribs")) in
          let dummy_params = scheme_sexpr_list_of_sexpr_list 
          (List.map (fun (var, _) -> ScmPair (ScmSymbol var,
           ScmPair (ScmPair (ScmSymbol "quote", ScmPair (ScmSymbol "whatever", ScmNil)),
            ScmNil))) ribs) in
        let set_exprs = List.map (fun (var, expr) -> ScmPair(ScmSymbol "set!", ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)))) ribs in
        let body = List.fold_right(fun car cdr -> ScmPair(car,cdr)) set_exprs exprs in
  ...
  let annotate_tail_calls = 
    let rec run in_tail = function
      ...
      | ScmSeq' (exprs) ->
        ...
        let last = run in_tail (List.hd exprs') in
        let rest = List.rev (List.map (fun exp -> run false exp) (List.tl exprs')) in
        ScmSeq' (rest @ [last])
      (* add support for or *)
      | ScmOr' [] -> ScmOr' []
...
module Code_Generation (* : CODE_GENERATION *) = struct
  ...
  let const_repr sexpr loc table = match sexpr with
    ...
    | ScmString str ->
      ...
      let ascii = ASCII str in
       ([RTTI "T_string"; Quad str_len; ascii], 1 + word_size + str_len)
    | ScmSymbol sym ->
    ...
    | ScmNumber (ScmFraction (numerator, denominator)) ->
       ([RTTI "T_fraction"; Quad numerator; Quad denominator], 1 + word_size*2)
    | ScmNumber (ScmReal x) ->
       ([RTTI "T_real"; QuadFloat x], 1 + word_size)
    | ScmVector s ->
      let vec_len = List.length s in
      let int_list = List.map (fun sexpr -> search_constant_address sexpr table) s in
      let var_list = List.map (fun var -> Quad var) int_list in
       ([RTTI "T_vector"; Quad vec_len]@var_list, 1 + word_size+ vec_len*word_size)
    | ScmPair (car, cdr) ->
      let car_addr = search_constant_address car table in
      let cdr_addr = search_constant_address cdr table in
       ([RTTI "T_pair"; ConstPtr car_addr; ConstPtr cdr_addr], 1 + word_size*2)
  ...
  let make_tc_applic_recycle_frame_done =
    ...
    let make_lambda_opt_loop_copy_to_new_frame_exact =
      make_make_label ".L_lambda_opt_loop_copy_to_new_frame_exact";;
    
    let make_lambda_opt_loop_copy_to_new_frame_exact_end =
      make_make_label ".L_lambda_opt_loop_copy_to_new_frame_exact_end";;
    let make_lambda_opt_create_list_of_opt_params =
      make_make_label ".L_lambda_opt_create_list_of_opt_params";;
    let make_lambda_opt_create_list_of_opt_params_end =
      make_make_label ".L_lambda_opt_create_list_of_opt_params_end";;
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmLambda' (params', Simple, body) ->
         ...
         | ScmLambda' (params', Opt opt, body) ->
          ...
          and label_loop_fix_stack_more = make_lambda_opt_loop ()
          and label_loop_fix_stack_more_exit = make_lambda_opt_loop_exit () 
          and label_loop_copy_exact =  make_lambda_opt_loop_copy_to_new_frame_exact ()
          and label_loop_copy_exact_end = make_lambda_opt_loop_copy_to_new_frame_exact_end ()
          and label_create_list_opt = make_lambda_opt_create_list_of_opt_params ()
          and label_create_list_opt_end = make_lambda_opt_create_list_of_opt_params_end () in 
           "\tmov rdi, (1 + 8 + 8)\t; sob closure\n"
          ^ "\tcall malloc\n"
          ^ "\tpush rax\n"
          ^ (Printf.sprintf "\tmov rdi, 8 * %d\t; new rib\n" params)
          ...
          ^ (Printf.sprintf "\tcmp rsi, %d\n" params)
          ^ (Printf.sprintf "\tje %s\n" label_loop_params_end)
          (* the claculation for 8*4 is for skipping the old fp and ret and env and argc *)
          ^ "\tmov rdx, qword [rbp + 8 * rsi + 8 * 4]\n" 
          ^ "\tmov qword [rbx + 8 * rsi], rdx\n"
          ^ "\tinc rsi\n"
          ^ (Printf.sprintf "\tjmp %s\n" label_loop_params)
          ...
          ^ "\tmov qword [rax], rbx\t; ext_env[0] <-- new_rib \n"
          ^ "\tmov rbx, rax\n"
          (* rax will now hold the clouser address *)
          ^ "\tpop rax\n"
          ^ "\tmov byte [rax], T_closure\n"
          ^ "\tmov SOB_CLOSURE_ENV(rax), rbx\n"
          ^ (Printf.sprintf "\tmov SOB_CLOSURE_CODE(rax), %s\n" label_code)
      ...
      | ScmApplic' (proc, args, Non_Tail_Call) -> 
         ...
         | ScmApplic' (proc, args, Tail_Call) -> 
          ...
          ^ (Printf.sprintf "\tjmp %s\n" label_loop_copy)
          ^ (Printf.sprintf "%s:\n" label_loop_copy_end)
          (* Align the stack pointer correctly *)
          ^ "\tlea rsp, [r8 + 8]\n"
