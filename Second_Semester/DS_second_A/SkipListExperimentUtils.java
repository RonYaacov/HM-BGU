...
public class SkipListExperimentUtils {
    ...
    public static double measureDeletions(AbstractSkipList skipList, int size) {
        ...
        for (Integer item : items) {
            deletedItems++;
            curr = skipList.find(item);
            skipList.delete(curr);
            }
        

        double endTime = System.nanoTime();
        double AvregeInsertTime = (endTime - startTime)/deletedItems; 
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0 ; i < p.size() ; i++){ 
            for (int j =0 ; j < x.size() ; j++){
                ...
                System.out.println("p: "+p.get(i)+", x: "+x.get(j));
                for (int d=0 ; d < levels.size() ; d++){
        ...
        for (int m = 0 ; m < p.size() ; m++){ 
            for (int n = 0 ; n< x2.size() ; n++){
                for (int q = 0; q < 30; q++) {
                    ...
                    double averageInsertionTime = totalInsertTime / 30;
